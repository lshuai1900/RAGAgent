...
本阶段只产出文档与规则，不写任何业务代码、不创建项目骨架、不实现 FastAPI 路由 / SQLAlchemy Model / Alembic 迁移 / Milvus / Embedding / LLM / RAG / 文档解析 / SSE / Memory / TraceSpan 写入。
...
**严格三批推进纪律**：每批完成后必须通过验收检查，才能启动下一批。禁止一次性铺开 T1-T18，禁止提前实现后续批次功能。
